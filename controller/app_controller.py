...
class AppController:
    ...
    def __init__(self, username: str, port: int):
        ...
        self.ui_on_fingerprint:   Optional[Callable[[str, str, bool], None]] = None  # peer_id, fingerprint, is_match
    ...
    async def start(self):
        ...
        """
        logger.info("=" * 80)
        logger.info("[START] INICIANDO APLICACIÓN P2P")
        logger.info("=" * 80)
        ...
        logger.info("[CTRL] [WAIT] Iniciando servidor WebSocket...")
        
        await self.node.start()
        
        logger.info("[CTRL] [OK] Servidor WebSocket iniciado correctamente")
        logger.info("=" * 80)
    ...
    async def connect_to_peer(self, ip: str, port: int):
        ...
        """
        logger.info("=" * 80)
        logger.info(f"[CTRL] [CONN] INICIANDO CONEXIÓN A PEER")
        logger.info("=" * 80)
        
        ok, err = validate_connection(ip, port)
        if not ok:
            logger.error(f"[CTRL] [ERR] Validación fallida: {err}")
            if self.ui_on_error:
        ...
        try:
            ...
            logger.info("[CTRL] [OK] Conexión completada")
        except ConnectionError as e:
            logger.error(f"[CTRL] [ERR] Error de conexión: {e}")
            if self.ui_on_error:
                self.ui_on_error(str(e))
        except Exception as e:
            logger.error(f"[CTRL] [ERR] Error inesperado: {e}")
            if self.ui_on_error:
    ...
    async def send_message(self, peer_id: str, content: str):
        ...
        msg = Message(
            ...
            timestamp=datetime.now(timezone.utc), receiver_id=peer_id,
    ...
    def _on_peer_connected(self, peer: Peer):
        logger.info("=" * 80)
        logger.info(f"[CTRL] [OK] PEER CONECTADO")
        logger.info("=" * 80)
        ...
        session = self.node._crypto.get(peer.id)
        if session and session.peer_fingerprint:
            stored = self.db.get_fingerprint(peer.id)
            if stored and stored != session.peer_fingerprint:
                logger.warning(f"[CTRL] [WARN] Fingerprint cambiado para {peer.username}!")
            is_match = (stored == session.peer_fingerprint) if stored else False
            if not stored:
                self.db.save_fingerprint(peer.id, session.peer_fingerprint)
                is_match = True  # primera vez, confiar automáticamente
            if self.ui_on_fingerprint:
                self.ui_on_fingerprint(peer.id, session.peer_fingerprint, is_match)
        
        logger.info("[CTRL] Cargado historial mensajes")
        logger.info("=" * 80)
    ...
    def _on_peer_disconnected(self, peer: Peer):
        logger.warning("=" * 80)
        logger.warning(f"[CTRL] [ERR] PEER DESCONECTADO")
        logger.warning("=" * 80)
    ...
    def _on_message_received(self, sender_id, sender_name, content, timestamp):
        msg = Message(
            ...
            content=content, timestamp=datetime.now(timezone.utc), receiver_id=self.node.peer_id,
    ...
    async def start_videocall(self, peer_id: str):
        ...
        if not peer:
            logger.error(f"[VIDEOCALL] [ERR] Peer {peer_id} no encontrado")
            return
    ...
    async def accept_videocall(self, peer_id: str):
        ...
        logger.info("=" * 80)
        logger.info(f"[VIDEOCALL] [CALL] ACEPTANDO VIDEOLLAMADA de {peer.username}")
        logger.info("=" * 80)
    ...
    async def reject_videocall(self, peer_id: str):
        ...
        self._pending_videocall_offers.pop(peer_id, None)
        logger.info(f"[VIDEOCALL] [ERR] Videollamada rechazada de {peer_id}")

    async def end_videocall(self, peer_id: str):
        """Cuelga una llamada activa y notifica al peer."""
        logger.info(f"[VIDEOCALL] [END] Colgando videollamada con {peer_id}")
        self.videocall.stop()
    ...
    def _start_videocall_session(self, peer):
        ...
        logger.info(f"[VIDEOCALL] [OK] Sesión iniciada con {peer.username}")

    def _on_videocall_ended(self):
        """Callback interno cuando VideoCallWS termina sus loops."""
        logger.info("[VIDEOCALL] [DONE] Sesión de videollamada terminada")
    ...
    async def _send_videocall_signal(self, peer_id: str, signal_type: str):
        ...
        if not peer or not session:
            logger.error(f"[VIDEOCALL] [ERR] No se puede enviar señal '{signal_type}': peer/crypto no disponible")
            return
        try:
            ...
            logger.info(f"[VIDEOCALL] [OUT] Señal enviada: {signal_type} → {peer.username}")
        except Exception as e:
            logger.error(f"[VIDEOCALL] [ERR] Error enviando señal '{signal_type}': {e}")
    ...
    async def handle_incoming_videocall_message(self, peer, message):
        ...
        if msg_type == "videocall_offer":
            ...
            logger.info(f"[VIDEOCALL] [IN] Oferta recibida de {peer.username}")
            ...
            logger.info(f"[VIDEOCALL] [OK] {peer.username} aceptó la llamada")

        elif msg_type == "videocall_end":
            # El peer colgó
            logger.info(f"[VIDEOCALL] [END] {peer.username} colgó la llamada")
            self.videocall.stop()
