...
class AppController:
    """
    Única clase que conoce tanto la capa de red como la capa de UI.
    La red llama a métodos privados _on_*.
    La UI se engancha a los atributos ui_on_*.
    """
    ...
    def __init__(self, username: str, port: int):
        ...
        # Historial en memoria: peer_id → lista de mensajes
        ...
        # ── Callbacks para la UI (se asignan desde MainWindow) ─────────────
    ...
    async def connect_to_peer(self, ip: str, port: int):
        ...
        if not ok:
            logger.warning(f"[CTRL] Validación fallida: {err}")
            if self.ui_on_error:
    ...
    async def send_message(self, peer_id: str, content: str):
        ...
        msg = Message(
            id=msg_id,
            sender_id=self.node.peer_id,
            sender_name=self.node.username,
            content=content,
    ...
    # ── Callbacks del nodo (hilo asyncio) ───────────────────────────────────

    def _on_peer_connected(self, peer: Peer):
        logger.info(f"[CTRL] Peer conectado: {peer.username}")
        self.db.upsert_peer(peer)
        self.db.log_connection(peer.id, "connected")
        # Cargar historial persistido si aún no está en memoria
        if peer.id not in self._messages:
            history = self.db.get_messages_for_peer(peer.id, self.node.peer_id)
            for msg in history:
                # Rellenar sender_name desde peers conocidos
                if not msg.is_mine:
                    msg.sender_name = peer.username
                else:
                    msg.sender_name = self.node.username
    ...
    def _on_peer_disconnected(self, peer: Peer):
        logger.info(f"[CTRL] Peer desconectado: {peer.username}")
